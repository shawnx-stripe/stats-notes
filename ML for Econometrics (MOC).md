...
- Policy learning/uplift
  - Learn treatment assignment rules maximizing expected outcomes; uplift trees/forests, policy gradients (placeholders)

> [!warning] Leakage
> Do not use post-treatment or future information in nuisance/feature construction; see [[bad controls]] and “leakage” (placeholder).
...
- Linear/regularized
  - [[regularization|Lasso/ridge/elastic net]], GLM/GBM with regularization, grouped lasso (placeholder)
- Tree ensembles
  - Random forests, gradient boosting (XGBoost/LightGBM/CatBoost) (placeholders)
- Kernel and margin methods
  - SVM, Gaussian processes (placeholders)
- Neural networks
  - MLP/CNN/RNN/LSTM/Transformers (placeholders)
- Model selection
  - Cross-validation, nested CV, information criteria for hybrids
- Interpretation
  - Partial dependence (PDP), ICE, SHAP/LIME (placeholders); stability selection (placeholder)
...
- Heterogeneous treatment effects (HTE)
  - [[treatment effect heterogeneity]]; CATE learners (meta-learners): T-/S-/X-/R-/DR- learners (placeholders)
  - Causal trees/forests (grf/causal forest) (placeholder)
- Policy learning / uplift
  - Uplift trees/forests, doubly robust policy evaluation/learning, bandits (contextual bandits) (placeholders)
- Design-aware ML
  ...
  - IV with ML: ML for first-stage, weak-IV robust inferences; DeepIV/IV-forest (placeholders)
...
- Cross-fitting and sample splitting
  - K-fold cross-fitting for orthogonal estimators; nested CV for hyperparams (placeholders)
...
- Evaluation metrics
  ...
  - CATE/Policy: policy value, regret, Qini/uplift metrics (placeholders); influence-function-based SEs
...
- Missing data
  - Imputation (mice/softImpute/Kalman) (placeholders); [[Inverse Probability of Censoring Weighting (IPCW)|IPCW]] for missing outcomes; [[Inverse Probability Weighting (IPW)|IPW]]/[[entropy balancing]] for selection/design
...
- Bootstrap
  - Cluster/bootstrap for panels; block bootstrap for series (placeholders)
- Conformal prediction
  - Conformal intervals for predictions and residuals (placeholder); conformalized quantile regression
...
- Fairness
  - Group/individual fairness metrics and constraints (placeholders)
- Stability and transport
  - Covariate shift and domain adaptation; reweighting/importance sampling (placeholders)
- Transparency
...
- Julia/Stata: EconPDEs/MLJ (Julia) (placeholder); teffects/causal inference add-ons (Stata)
...
- [[few-cluster corrections]] · feature engineering (placeholder) · [[fairness]]  
